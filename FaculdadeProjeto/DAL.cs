...
namespace FaculdadeProjeto
{
    internal class DAL
    {
        private static String strConexao = "Server=DESKTOP-71HQBEU\\SQLEXPRESS;Database=Faculdade;Trusted_Connection=True;TrustServerCertificate=True;";
        ...
        {
            conecta();
            String aux = "SELECT * FROM Aluno WHERE nm_email_aluno = @nm_email_aluno AND cd_senha_aluno = @cd_senha_aluno";
            strSQL = new SqlCommand(aux, conn);
            ...
            if (result.Read())
            {
                ...
                Erro.setMsg("Email ou senha incorretos.");
            desconecta();
        }
